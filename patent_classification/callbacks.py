...
class CallbackBase(tf.keras.callbacks.Callback):
    ...
    def load_previous_run(self):
        if os.path.exists(os.path.join(self.exp_dir, "metrics_%s.csv" % self.split_id)):
            self.logger.info("found previous run %s " % os.path.join(self.exp_dir, "metrics_%s.csv" % self.split_id))

            df_metrics = pd.read_csv(os.path.join(self.exp_dir, "metrics_%s.csv" % self.split_id))
            ...
            self.logger.info("init self.epochs_since_best_f1 - %s " % self.epochs_since_best_f1)
            self.logger.info("init self.epochs_since_best_f1 - %s " % self.best_f1)
    ...
    def save_for_split(self, X, y_actual, ids, split_name, epoch):
        ...
        df_pred[columns].to_csv(os.path.join(self.exp_dir, "pred-output", "best_pred_%s.csv" % split_name), index=False)
        df_pred[columns].to_csv(os.path.join(self.exp_dir, "pred-output", "epoch_pred_%s_%s.csv" % (split_name, epoch)), index=False)

        df_actual = pd.DataFrame(y_actual, columns=self.mlb.classes_)
        df_actual["id"] = ids[:y_pred.shape[0]]
        df_actual[columns].to_csv(os.path.join(self.exp_dir, "pred-output", "actual_%s.csv" % (split_name)), index=False)

    def save_epoch_results(self, epoch):
        
        self.save_for_split(self.X_test, self.y_test, self.test_ids, "test", epoch)
        
        if self.heldout_ids is not None:
            self.save_for_split(self.X_heldout, self.y_heldout, self.heldout_ids, "heldout", epoch)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        bert_layer_list = [layer for layer in self.model.layers if "bert" in layer.name]

        if len(bert_layer_list):
            bert_layer_list[0].save_pretrained(os.path.join(self.exp_dir, "bert_model"))
        ...
        metric_dev = calc_metrics(self.y_dev, y_dev_pred, 'dev-macro-calc', split_index=0)
        metric_dev["mode"] = "dev"
            ...
            fileW.write(
                "\n".join([":".join([str(item[0]), ",".join(item[1]), ",".join(item[2])])
                           for item in zip(self.dev_ids,
                                           self.mlb.inverse_transform(self.y_dev),
                                           self.mlb.inverse_transform(y_dev_pred)
                                           )])
            )
        ...
        self.best_model.save_weights(os.path.join(self.exp_dir, "best-model.h5"))
        ...
        metric_train = calc_metrics(self.transform_pred(
            self.y_train), y_train_pred, 'train-macro-calc', split_index=0)
...
class CallbackTMM(CallbackBase):
    ...
    def __init__(self, 
        ...
        Args:
            X_train (tensor): Feature Vector train
            y_train (tensor): label binarizer for train 
            X_test (te): Feature Vector test
            y_test (_type_): label binarizer for test
            X_dev (_type_): Feature Vector dev
            y_dev (_type_): label binarizer for dev
            X_test_holdout (_type_): Feature Vector heldout
            y_test_holdout (_type_): label binarizer for heldout
            train_ids (list): train ids
            test_ids (list): test ids
            dev_ids (list): dev ids
            heldout_ids (list): heldout ids
            mlb (MultiLabelBinarizer): Binarizer
            exp_dir (str): Experiment Directory path
            split_id (int): Split index in case of cross-validation setup
            patience (int, optional): Number of epochs to check for a better model. Defaults to 5.
    ...
    def transform_pred(self, y):
        """
        Get MultiLabelBinarized values for predicted y values which contains a binary pred for each label.

        Args:
            y (tensor): A prediction for each label.

        Returns:
            tensor: binarized values.
        """
...
# class CallbackSingleTask(CallbackBase):
...
#             train_ids, test_ids, dev_ids, heldout_ids,
#             mlb, exp_dir, split_id, patience=5, logger=None):
#         super(CallbackSingleTask, self).__init__(
#                                                 X_train, y_train, 
#                                                 X_test, y_test,
#                                                 X_dev, y_dev,
#                                                 X_test_holdout, y_test_holdout,
...
#     def set_best_threshold(self, y, y_pred_prob):
#         thresholds = [0.05 * index for index in range(1, 20)]
#         perf_list = list()
#         for threshold in thresholds:
#             y_pred_prob_thres = y_pred_prob > threshold
#             perf = calc_metrics(y, y_pred_prob_thres)
#             perf["threshold"] = threshold
#             perf_list.append(perf)
#         df_metrics = pd.DataFrame(perf_list)
#         threshold_index = df_metrics.f1_macro.idxmax()
#         self.epoch_threshold = df_metrics["threshold"].iloc[threshold_index]
